...
const DashboardHome = () => {
  ...
  return (
    <div className="space-y-1">
      {/* 1. Header: username + cash + points + referral + withdraw */}
      <div className="flex items-center justify-between px-1 flex-wrap gap-0.5">
        <div className="flex items-center gap-1">
          <span className="text-[9px] font-semibold">Hi, <span className="text-primary">{profile.first_name || profile.username}</span></span>
          {profile.is_verified ? <CheckCircle className="h-2 w-2 text-success" /> : <AlertCircle className="h-2 w-2 text-warning" />}
        </div>
        <div className="flex items-center gap-1 flex-wrap">
          <span className="text-primary text-[9px] font-bold">${Number(profile.cash_balance).toFixed(2)}</span>
          <span className="text-[7px] text-muted-foreground">|</span>
          <span className="text-info text-[9px] font-bold">{profile.points} pts</span>
          <span className="text-[7px] text-muted-foreground">|</span>
          ...
          <Link to="/dashboard/withdrawal"><Button size="sm" className="h-4 text-[7px] px-1.5 rounded">Withdraw</Button></Link>
        </div>
      </div>
